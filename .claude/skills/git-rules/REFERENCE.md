# Git 運用リファレンス（canonical）

このファイルはリポジトリでの Git / PR の運用ルールの一次保存先です（canonical）。エージェントはこのファイルを参照してチェックや助言を行ってください。
...
## エージェント向けの使い方
- エージェントはこの `REFERENCE.md` を唯一の参照先として使用してください。
- PR チェックを自動で行う場合は、チェック項目と例文を提案する形で支援してください（自動で PR を block したり merge を実行するのではなく、助言を優先）。
